...
function _gaLt(event) {
  ...
  if (el && el.href) {
    if (el.href.indexOf(location.host) == -1 && el.href != "javascript:;") {
      /* external link */
      var eventOptions = {
      ...
      console.log(
        "ga event logged c: Outbound Link a: " + el.href + " l:" + el.text
      );
      ...
      var target =
        el.target && !el.target.match(/^_(self|parent|top)$/i)
          ...
          : false;
      var newWindow =
        event.button == 1 || event.metaKey || event.ctrlKey || target;
