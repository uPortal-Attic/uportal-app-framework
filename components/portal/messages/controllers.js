...
define(['angular'], function(angular) {
  return angular
      ...
      .controller('MessagesController', [
         ...
         */
          var filterMessages = function(allMessages) {
            var dateFilterMessages = $filter('filterByDate')(allMessages);
            var groupPromise = promiseMessagesByGroup(dateFilterMessages);

            $q.all([promiseSeenMessageIds(), groupPromise])
         ...
         */
          ...
          var dataMessageSuccess = function(result) {
            $scope.messages = $filter('separateMessageTypes')(result);
            $scope.hasMessages = true;
          };
      ...
      .controller('NotificationsController', [
         ...
         */
          var notificationChange = $rootScope.$on(
              'notificationChange',
              function() {
            ...
            // Save all seenMessageIds for later
              ...
              angular.forEach(separatedNotifications.seen, function(message) {
                if (message.recurrence) {
                  var index = separatedNotifications.seen.indexOf(message);
