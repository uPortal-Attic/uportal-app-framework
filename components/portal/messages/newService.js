  ...
  return angular.module('portal.messages.services', [])
    .factory('messagesService', [
      ...
      function($http,
        ...
        var GET_DATA_MESSAGES_FAILED = 'Could not process urls in messages.';
        // //////////////////
        // Exposed methods //
        // //////////////////
        ...
        var getAllNotifications = function() {
            getAllMessages()
               .then(function(result) {
                var separatedMessages = $filter('separateMessageTypes')(
                    result
                );
                var nonPriorityNotifications = $filter('priorities')(
                    separatedMessages.notifications
                );
                return nonPriorityNotifications.low;
            })
            .catch(function(error) {
                $log.warn(error);
            });
        ...
        var getAllPriorityNotifications = function() {
            getAllMessages()
               .then(function(result) {
                var separatedMessages = $filter('separateMessageTypes')(
                    result
                ...
                return nonPriorityNotifications.high;
            })
            .catch(function(error) {
                $log.warn(error);
            });
        ...
        var getMessagesByDate = function(messages) {
          return $filter('filterByDate')(messages);
        };
        ...
        var markMessageSeen = function(message) {
            var seenIds = [];
            getSeenMessageIds()
              .then(function(result) {
                  seenIds = result;
                  if (seenIds.indexOf(message.id) == -1) {
              ...
              updateSeenIds(seenIds);
          };

          var markMessageUnseen = function(message) {
              var seenIds = [];
              getSeenMessageIds()
                  .then(function(result) {
                      seenIds = result;
          ...
          var updateSeenIds = function(seenIds) {
        ...
        return {
            ...
            getAllPriorityNotifications: getAllPriorityNotifications,
            getSeenMessageIds: getSeenMessageIds,
            markMessageSeen: markMessageSeen,
            markMessageUnseen: markMessageUnseen,
          };
      }]);
  });
