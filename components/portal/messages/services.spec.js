...
define(['angular-mocks', 'portal'], function() {
  describe('MessagesService', function() {
    ...
    beforeEach(inject(function(_messagesService_, _$httpBackend_, SERVICE_LOC) {
      ...
      if (loginSilentUrl) {
        httpBackend.whenGET(loginSilentUrl).respond({
          status: 'success',
          username: 'admin',
        });
      }
    ...
    it(
        'message should not have title from data if specified' +
        'but url is not used',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
    ...
    it(
        'message should not have more info url from data if specified, but' +
        'learnMoreButton not configured',
        function() {
          var url = 'exampleUrl';
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
    ...
    it(
        'message should not have more info url from data if specified,' +
        'but no dataurl is configured',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
              {
          ...
          messagesService
              ...
              .then(function(dataMessages) {
                expect(dataMessages).toBeTruthy();
                expect(dataMessages.length).toEqual(2);
                expect(dataMessages[1].moreInfoButton.url).toEqual(
                    'to be replaced'
                );
              });
          httpBackend.flush();
    ...
    it(
        'message should not appear if dataURL is' + 'present but incorrect',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
              {
    ...
    it(
        'message should appear if dataURL is present' + 'and returns data',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
              {
    ...
    it(
        'message should appear if dataURL is present and returns data ' +
        'specifically asked for by dataObject',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
              {
    ...
    it(
        'message should not appear if dataURL is present and cannot' +
        'return data specifically asked for by dataObject',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
              {
    ...
    it(
        'message should appear if dataURL is not present and dataObject' +
        'is mistakenly present',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
              {
    ...
    it(
        'message should appear if dataURL is present and' +
        'return data specifically asked for by dataArray' +
        'and searched by object',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
              {
    ...
    it(
        'message should not appear if dataURL is present and returns data' +
        'specifically asked for by dataArray with two filters and searched by' +
        'object when filter does not match',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
              {
    ...
    it(
        'message should not appear if dataURL is present and attempts to' +
        'arrayFilter on non-array',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
              {
    ...
    it(
        'message should appear when dataObject is present and' + 'not an array',
        function() {
          httpBackend.whenGET(messagesUrl).respond({
            messages: [
              {
