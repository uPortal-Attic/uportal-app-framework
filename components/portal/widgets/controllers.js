                    ...
                    widgetService
                        ...
                        .then(function(links) {
                          if (!(links.content && links.content.links)) {
                            $log.warn(
                                'Undefined dynamic links content for ' +
                            widget.fname +
            ...
            $scope.triggerRemoveButton = function(event) {
              ...
              if (event.keyCode === 13) {
                var removeButton = angular.element(
                    event.target.querySelector('.widget-remove')
                );
            ...
            // Initialize scope variables
              ...
              // Get widget data for provided app (fname)
                widgetService
                    ...
                    .catch(function(error) {
                      $log.warn(
                          'WidgetCardController couldn\'t get data for: ' + fname
                      );
                      $log.error(error);
                    });
              } else {
    ...
    // External Message Controller
        .controller('WidgetExternalMessageController', [
          ...
          function($scope, $log, widgetService) {
            var widget = $scope.widget;
            widgetService
                .getWidgetExternalMessage(widget)
                .then(function(externalMessage) {
                ...
                .catch(function() {
                  $log.warn(
                      'Could not get external widget message for ' + widget.fname
                  );
                });
          },
        ])
                    ...
                    .then(function(data) {
                      if (data) {
                        if (config.singleElement) {
                          ...
                          widget.selectedUrl = widget.widgetData[config.value];
                        } else if (
                          widget.widgetData[config.arrayName] &&
           ...
           */
            ...
            // Trigger loading spinner
              ...
              // Make sure config has values
                ...
                widgetService
                    .getRssAsJson($scope.widget.widgetURL)
        ...
        .controller('ActionItemsController', [
           ...
           */
            ...
            // Get number from provided url
              widgetService
                  .getActionItemQuantity(item.feedUrl)
                  .then(function(data) {
                  ...
                  .catch(function(error) {
                    ...
                    $log.warn(
                        'ACTION ITEMS CONTROLLER: ' +
                    'Problem getting action item data from: ' +
                    item.feedUrl
                    );
           ...
           */
            ...
            // For each entry in actionItems, get the number of items
              ...
              // Make sure the current item has required fields configured
                if (
                  $scope.config.actionItems[i].textSingular &&
                ...
                if (i === $scope.config.actionItems.length - 1) {
                  $scope.actionItems = $filter('orderBy')(
                      $scope.actionItems,
                      'quantity',
                      true
                  );
           ...
           */
            ...
            // Initialize bindable members
              ...
              $scope.itemsLimit = 3;
              $scope.launchText = $scope.config.launchText
              ? $scope.config.launchText
              : 'See all';

              // Make sure we got a widget and necessary config
              if (
                $scope.widget &&
              $scope.config.actionItems &&
        ...
        .controller('TimeSensitiveContentController', [
          ...
          function($scope, $filter, $log) {
            ...
            var indexWithinTemplateRange = function(now, callsToAction) {
              ...
              angular.forEach(callsToAction, function(callToAction, index) {
                if (result === -1) {
                // We haven't found a call to action
                  var startDate = resolveDate(
                      callToAction.activeDateRange.templateLiveDate
                  );
                  var endDate = resolveDate(
                      callToAction.activeDateRange.templateRetireDate,
                      true
                  );
            ...
            var resolveTemplateStatus = function(now, actionStart, actionEnd) {
              if (
                ...
                throw new TypeError(
                    'now, actionStart, and actionEnd must all be set'
                );
              }
              ...
              var statuses = [
                ...
                },
                {
                  ...
                  check: function() {
                    return (
                      moment(now).isBetween(actionStart, actionEnd) &&
           ...
           */
            var displayTimeSensitiveContent = function(config) {
              ...
              $scope.templateStatus = resolveTemplateStatus(
                  now,
                  actionStart,
                  actionEnd
              );
              ...
              // so load that dynamic JSON, do the parsing and selecting
                widgetService
                   ...
                   */
                      ...
                      var caseToActivate = $scope.switchConfig.cases.find(
                          matchesValue
                      );
                    ...
                    .catch(function(error) {
                      ...
                      $scope.widget.fname +
                      ' errored fetching or processing JSON [' +
                      $scope.widget.widgetURL +
        ...
        .controller('WeatherWidgetController', [
           ...
           */
            ...
            // Declare asynchronous promises to be executed in $q.all()
              ...
              $q.all([widgetPromise, userPreferencesPromise])
                  .then(function(data) {
                  ...
                  data[1] &&
                  (!data[0].errors ||
                    (data[0].errors && 0 >= data[0].errors.length))
                    ) {
                      // Set local variables
           ...
           */
            var convertCelsiusToFahrenheit = function() {
              ...
              for (var i = 0; i < $scope.weatherData.length; i++) {
                $scope.weatherData[i].currentWeather.temperature =
                $scope.weatherData[i].currentWeather.temperature * ratio +
                offset;

                for (var j = 0; j < $scope.weatherData[i].forecast.length; j++) {
                  $scope.weatherData[i].forecast[j].highTemperature =
                  $scope.weatherData[i].forecast[j].highTemperature * ratio +
                  offset;
                  $scope.weatherData[i].forecast[j].lowTemperature =
                  $scope.weatherData[i].forecast[j].lowTemperature * ratio +
                  offset;
                }
              }
            };
